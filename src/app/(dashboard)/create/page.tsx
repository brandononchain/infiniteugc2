...
/* ─── Step Indicator ─── */
const STEPS = ["Name", "Avatar", "Script", "Customize"];
      ...
      {STEPS.map((step, i) => (
        <div key={step} className="flex items-center">
          <div className="flex flex-col items-center gap-1.5">
            ...
            <span className={`text-[10px] font-medium ${i <= current ? "text-zinc-700" : "text-zinc-400"}`}>
              {step}
            </span>
          </div>
          {i < STEPS.length - 1 && (
            <div className={`w-16 h-px mx-2 mb-5 ${i < current ? "bg-accent-400" : "bg-zinc-200"}`} />
          )}
        </div>
      ))}
    </div>
          ...
          <div className="space-y-8">
            ...
            <section className="space-y-5">
              <div className="flex items-center gap-2.5">
                ...
                <h2 className="text-base font-bold text-zinc-950">Write Script</h2>
              </div>

              <div>
                <label className="text-xs font-semibold text-zinc-700 mb-1.5 block">Script</label>
                <div className="bg-amber-50/60 border border-amber-200/40 rounded-xl p-4 flex items-start gap-3">
                  <Info size={16} className="text-amber-500 mt-0.5 shrink-0" />
                ...
                <p className="text-[10px] text-zinc-400 mt-1.5">Choose or create the script your avatar will speak.</p>
              </div>
            </section>
                ...
                <div className="bg-white border border-zinc-200/60 rounded-xl p-4">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    >
                      Enable
                      {captionsEnabled ? (
                        <ToggleRight size={24} weight="fill" className="text-accent-500" />
                      ) : (
                        <ToggleLeft size={24} weight="regular" className="text-zinc-300" />
                      )}
                    </button>
                  </div>
