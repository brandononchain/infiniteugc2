      ...
      <div className="sticky top-0 z-30 brutal-header">
        <div className="max-w-5xl mx-auto px-6 lg:px-10 h-16 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-zinc-500">Clone and manage your AI voices</p>
          </div>
          <button
      ...
      <div className="max-w-5xl mx-auto px-6 lg:px-10 py-8">
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {voices.map((voice) => (
              <div key={voice.id} className="bg-white rounded-xl p-4 brutal-card">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-violet-50 flex items-center justify-center">
                    <SpeakerHigh size={18} weight="duotone" className="text-violet-500" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-bold text-zinc-900">{voice.name}</p>
          ...
          <div className="bg-white rounded-xl p-16 text-center brutal-empty">
            ...
            <p className="text-xs text-zinc-500 mb-1">Clone your first voice by uploading an audio sample.</p>
            <p className="text-xs text-zinc-500 mb-6">Your cloned voices will appear here.</p>
            <button
              onClick={() => setShowModal(true)}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-xl border border-zinc-200">
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-sm font-bold text-zinc-950">Clone Voice</h2>
            ...
            {error && (
              <div className="mb-4 bg-rose-50 border border-rose-200 text-rose-700 text-xs font-medium px-3 py-2 rounded-lg">
                {error}
              </div>
            )}
            ...
            <div className="space-y-4">
              <div>
                <label className="text-[11px] font-semibold text-zinc-700 mb-1 block">Voice Name</label>
                <input
                  type="text"
                  value={cloneName}
              ...
              <div>
                <label className="text-[11px] font-semibold text-zinc-700 mb-1 block">Audio Sample</label>
                ...
                {audioFile ? (
                  <div className="flex items-center gap-2 p-3 bg-zinc-50 border border-zinc-200 rounded-lg">
                    <SpeakerHigh size={16} className="text-zinc-500" />
                    <span className="text-xs text-zinc-700 flex-1 truncate">{audioFile.name}</span>
                    <button onClick={() => setAudioFile(null)} className="text-zinc-400 hover:text-zinc-600">
                      <X size={12} />
                    </button>
                  </div>
                ) : (
                  <button
                    ...
                    className="w-full brutal-empty p-4 flex flex-col items-center gap-1.5 hover:border-accent-300 transition-all cursor-pointer"
                  >
                    <Upload size={18} className="text-zinc-400" />
                    <p className="text-[11px] font-semibold text-accent-600">Upload audio file</p>
                    <p className="text-[10px] text-zinc-500">MP3, WAV, M4A (10s–5min)</p>
                  </button>
                )}
              </div>
            </div>
            ...
            <div className="flex justify-end gap-3 mt-6">
              <button
                ...
                className="text-xs font-semibold text-zinc-600 px-4 py-2"
              >
                Cancel
              </button>
              <button
